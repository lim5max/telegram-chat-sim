...
import { R as Route$7, u as useChatsStore } from "./router-DwIrenfy.mjs";
...
const initialPrivate = [{
  ...
  from: "bot",
  time: "09:42",
  text: "👋 Привет! Я — ChatLogix.\n\nЗнакомая ситуация? Утром открываешь Telegram — а там 500+ непрочитанных в рабочих чатах. Листать всё — долго, пропустить важное — рискованно.\n\nЯ решаю эту проблему:\n\n📝 Саммари чата — каждое утро присылаю короткую выжимку: кто что обсуждал, какие решения приняли, что важно. Прямо в чат.\n\n🎤 Транскрипция голосовых — перевожу голосовые в текст автоматически. Не нужно слушать 5-минутные кружочки.\n\n🛡 Антиспам — удаляю спам, рекламу и флуд. Чат остаётся чистым.\n\n🎭 Анонимные сообщения — участники пишут через бота, автор скрыт от всех.\n\nУже 5 000+ чатов подключено. С чего начнём?",
  buttons: [{
    label: "⚙️ Управлять и улучшить свой чат",
    action: "onboard-admin"
  }, {
    label: "🧠 Получать выжимку по чатам в личку",
    action: "onboard-user"
  }]
}];
...
Напиши анонимное сообщение. Никто не узнает автора 🤫`
  ...
  const handleChatPicked = (chatId) => {
    ...
    if (!target) return;
    const topicSummary = target.topics.map((t) => `📌 ${t}
— Обсуждение (${Math.floor(Math.random() * 150 + 30)} сообщ.)`).join("\n\n");
    pushBot({
      text: `Готово 👌 ChatLogix добавлен в «${target.name}»!

Вот саммари по последним сообщениям:

📝 Саммари за сегодня — «${target.name}»

${topicSummary}

🔢 ${target.used} сообщ./день · ${target.members.toLocaleString("ru")} участников`
    });
    setTimeout(() => {
      pushBot({
        text: "Завтра тоже автоматически пришлю такое в чат.\n\nА пока — вот что ещё можно настроить 👇",
        buttons: [{
          label: "⚙️ Настроить саммари",
          action: "onboard-admin-settings"
        }, {
          label: "🎤 Включить транскрипцию голосовых",
          action: `onboard-admin-voice:${chatId}`
        }, {
          label: "🛡 Включить антиспам",
          action: `onboard-admin-antispam:${chatId}`
        }]
      });
    }, 1500);
  };
  const handleAction = (action, label) => {
    ...
    switch (action) {
      ...
      case "onboard-admin":
        pushBot({
          text: "Отлично! Покажу, как бот может усилить твой чат 👇\n\nВот что ChatLogix делает каждый день автоматически:\n\n📝 Саммари за 28 апреля — «Курьеры Москва»\n\n📌 Работа курьером\n— Обсуждали ставки: пик 380₽/час в центре\n— Новый бонус за ночные смены ×2\n\n📌 Яндекс приложение\n— Обновление интерфейса карты зон\n— Жалобы на задержку GPS-трекера\n\n📌 Точки доставки\n— Закрыли 3 новых точки на юге\n— Проблемы с парковкой у ТЦ\n\n🔢 342 сообщения · 1 248 участников\n\nТакое саммари приходит в чат каждое утро. Участникам не нужно листать сотни сообщений — всё ключевое в одном посте.\n\nХочешь так же в своём чате?",
          buttons: [{
      ...
      case "onboard-user": {
        ...
        const superText = summaryChats.slice(0, 4).map((c) => {
...
Хочешь получать такой каждый день?`,
      ...
      case "onboard-user-enable":
        ...
        pushBot({
          ...
          buttons: [{
            label: "🎧 Включить подкаст-версию",
            action: "onboard-user-podcast"
          }, {
            label: "🕵️ Анонимные сообщения",
            action: "onboard-user-anon"
          }, {
...
Доступно в ${anonAvailable.length} чатах: ${anonAvailable.map((c) => c.name).join(", ")}`,
          ...
          pushBot({
            text: "🕵️ Анонимные сообщения\n\nПиши в чат через бота — никто не узнает, кто автор. Пока нет чатов с включённой функцией. Попроси админа активировать.",
            buttons: [{
              label: "⚙️ Открыть настройки",
              action: "open-app"
            }]
...
Один выпуск в день, приходит в ЛС сразу после Super-Summary`,
      ...
      default:
        if (action.startsWith("onboard-admin-voice:")) {
          ...
          pushBot({
            text: `🎤 Транскрипция голосовых включена в «${target?.name}»!

Теперь все голосовые сообщения в чате будут автоматически переводиться в текст. Участникам больше не нужно слушать — просто читают.`,
            buttons: [{
              label: "⚙️ Настроить саммари",
              action: "onboard-admin-settings"
            }, {
