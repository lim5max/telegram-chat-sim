const PRODUCT_CHATLOGIX_REPORT = {
...
function buildReport(chat) {
  ...
  const memberFlow = FLOW_NET.map((n) => ({
    joined: n > 0 ? n * flowScale : 0,
    left: n < 0 ? -n * flowScale : 0
  }));
  const topicShare = [0.12, 0.08, 0.05];
  const topTopics = (chat.topics.length ? chat.topics : ["Общие обсуждения"]).slice(0, 3).map((title, i) => ({ title, count: Math.max(4, Math.round(totalMessages * topicShare[i])) }));
  ...
  const toxLevel = toxScore < 25 ? "low" : toxScore < 55 ? "medium" : "high";
  return {
    periodLabel: "29 мая — 12 июн",
    periodDays,
    totalMessages,
    ...
    podcast: chat.chatPodcast?.active ?? false ? { episodes: periodDays, longest: "последний выпуск (3:30)", avgDuration: "3:05" } : { episodes: 0, longest: "", avgDuration: "" },
    knowledgeBase: {
      searches,
    ...
    toxicity: {
      ...
      trend: "стабильно",
      note: toxLevel === "low" ? "Чат здоровый — токсичности почти нет." : "Есть всплески — стоит присмотреть за тоном общения."
    }
  };
}
const REPORTS = {
