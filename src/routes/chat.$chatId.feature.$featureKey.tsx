...
export const Route = createFileRoute("/chat/$chatId/feature/$featureKey")({
  ...
  notFoundComponent: () => <div className="p-8 text-center pb-32">Функция не найдена</div>,
  loader: ({ params }) => {
...
/* ------------------------------------------------------------------ */
/*  Summary settings (full spec from README2)                         */
/* ------------------------------------------------------------------ */
...
function FeatureSettings({ fk, chat }: { fk: FeatureKey; chat: Chat }) {
  ...
  switch (fk) {
    ...
    case "voice":
      return (
        <div className="glass-card rounded-[20px] p-4 space-y-3">
          ...
          <Setting label="Голос в текст">
            <Toggle
              defaultOn={chat.voice?.active}
              onChange={(v) => toast(v ? "Транскрипция включена" : "Транскрипция отключена")}
            />
          </Setting>
          <Setting label="Текущий тариф">
            <span className="text-[12px] font-semibold">{chat.voice?.plan ?? "Free"} Voice</span>
          </Setting>
          <Setting label="Лимит">
            {chat.voice?.plan === "Pro" ? "300 мин/мес" : chat.voice?.plan === "Ultra" ? "Безлимит" : "37.5 мин/мес"}
          </Setting>
          <button
            onClick={() => toast.success("Ссылка на оплату создана")}
            className="w-full py-2.5 text-[13px] font-semibold rounded-xl gradient-primary text-white"
          >
            {chat.voice?.plan === "Pro" ? "Перейти на Ultra" : "Перейти на Pro Voice — $16.99/мес"}
          </button>
        </div>
      );
    case "podcast": {
      ...
      return (
        <div className="glass-card rounded-[20px] p-4 space-y-3">
          ...
          <Setting label="Статус">
            <span
              className={`text-[12px] ${cp?.active ? "text-[oklch(0.85_0.15_155)]" : "text-muted-foreground"}`}
            >
              {cp?.status ?? "Не активен"}
            </span>
          </Setting>
        ...
        <div className="glass-card rounded-[20px] p-4 space-y-3">
          ...
          <div>
            ...
            <div className="mt-1 text-[11px] text-muted-foreground">
              {kb.quotaUsed} / {kb.quotaTotal} запросов использовано
            </div>
          </div>
          ...
          <button
            onClick={() => navigate({ to: "/" })}
            className="w-full py-2.5 text-[13px] font-semibold rounded-xl bg-white/10 hover:bg-white/15"
          >
            Открыть /search в чате
          </button>
        </div>
      );
    }
    ...
    case "anonymous":
      return (
        <div className="glass-card rounded-[20px] p-4 space-y-3">
          <SectionLabel>Настройки</SectionLabel>
          <Setting label="Отправлено сегодня">
            <span className="font-semibold">{chat.anonymous?.sentToday ?? 0}</span> / 3
          </Setting>
          ...
          <Setting label="Премодерация">
            <Toggle
              onChange={(v) =>
                toast(v ? "Премодерация включена" : "Премодерация отключена")
              }
            />
          </Setting>
        </div>
